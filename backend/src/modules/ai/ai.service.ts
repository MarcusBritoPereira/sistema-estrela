...
export class AiService {
  ...
  async processNaturalLanguageQuery(question: string): Promise<string> {
    ...
    try {
      if (
        ...
        return await this.explainRevenueDrop();
      }
      ...
      console.warn(
        '[Contingência] Banco offline, ativando IA mock em processNaturalLanguageQuery:',
        err,
      );
  ...
  private getMockAiResponse(question: string): string {
    const q = question.toLowerCase();
    if (q.includes('caiu') || q.includes('queda')) {
      return `🧠 **Diagnóstico Executivo de Contingência — Análise de Queda**\n\nConexão com SQL Server em modo offline. Pela última fotografia operacional, a variação semanal foi explicada por:\n1. Operador CARLOS (-5.4% de faturamento no Vale do Paraíba)\n2. Redução temporária de estoque no item 'ARROZ AGULHINHA 5KG'\n\n**Ação sugerida:** Acompanhar de perto a reposição e positivação das rotas afetadas.`;
    }
    if (
      q.includes('vendedor') ||
      q.includes('equipe') ||
      ...
      return `📊 **Análise da Equipe Comercial (Modo Contingência)**\n\nO operador destaque é **ALEXANDRE (Região Metropolitana)** com R$ 450.210,50 em 185 pedidos faturados.\n\n**Top 3 Operadores no Período:**\n1. ALEXANDRE: R$ 450.210,50 (185 pedidos)\n2. CARLOS: R$ 385.420,00 (162 pedidos)\n3. MARCELO: R$ 362.190,80 (154 pedidos)`;
    }
    if (
      q.includes('produto') ||
      q.includes('item') ||
      ...
      return `📦 **Curva A: Produtos de Maior Faturamento (Modo Contingência)**\n\n1. **ARROZ AGULHINHA TIPO 1 5KG**: R$ 362.500,00 | 14.500 unidades\n2. **LEITE CONDENSADO ESTRELA 395G**: R$ 198.800,00 | 28.400 unidades\n3. **CAFÉ TORRADO E MOÍDO 500G**: R$ 195.200,00 | 12.200 unidades`;
    }
    if (
      q.includes('faturamento') ||
      q.includes('mês') ||
      ...
      return `💰 **Resumo Financeiro e Comercial (Modo Contingência)**\n\n- **Faturamento Total Acumulado:** R$ 4.852.190,50\n- **Volume de Pedidos Faturados:** 1.820 pedidos\n- **Ticket Médio Geral:** R$ 2.666,03\n- **Faturamento no Mês de Maio:** R$ 2.154.210,00 (+16.4% em relação ao mês anterior)`;
    }
    if (q.includes('cliente') || q.includes('carteira')) {
      return `🤝 **Análise da Carteira de Clientes (Modo Contingência)**\n\nNos últimos 30 dias de operação gravada, a Distribuidora Estrela faturou para **580 clientes distintos**. A capilaridade nas rotas se mantém robusta e saudável.`;
    }
    return `🤖 **Assistente Analítico Estrela BI (Modo Contingência)**\n\nO banco de dados SQL Server real está inacessível no momento (VPN ou rede offline). Estou operando com nossa camada de contingência de alta fidelidade (R$ 4.852.190,50 acumulados em 1.820 pedidos faturados).\n\nVocê pode me perguntar sobre:\n- *"Quem foi o vendedor destaque?"*\n- *"Quais os produtos de maior faturamento?"*\n- *"Qual o faturamento do mês?"*`;
  }
  ...
  private async explainRevenueDrop(): Promise<string> {
    try {
      ...
      console.warn(
        '[Contingência] Banco offline, ativando IA mock em explainRevenueDrop:',
        err,
      );
      return this.getMockAiResponse('queda');
    }
  }
}
