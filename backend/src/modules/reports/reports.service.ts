...
export class ReportsService {
  ...
  private getDateWhereClause(prefix: string, dias?: number, startDate?: string, endDate?: string): string {
  ...
  async getConsolidadoFaturamento(dias: number = 30, startDate?: string, endDate?: string) {
  ...
  async getGiroEstoque(dias: number = 30, startDate?: string, endDate?: string) {
  ...
  async getDesempenhoEquipe(dias: number = 30, startDate?: string, endDate?: string) {
    const dateFilter = this.getDateWhereClause('', dias, startDate, endDate);
    const result = await this.pool.request().query(`
      SELECT
  ...
  async getCarteiraClientes(dias: number = 30, startDate?: string, endDate?: string) {
    const dateFilter = this.getDateWhereClause('', dias, startDate, endDate);
    const result = await this.pool.request().query(`
      SELECT TOP 100
